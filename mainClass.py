...
"""
This code will find the all the words in a haystack with a levenshtein distance
from the needle less than a certain amount

For each level have a list of sets of words that match up for that level
and that number of characters having been removed up to that point.
    one level per number in the distance variable.

For each letter in the needle, start at the top of the levels and 0 removed chars:
    -intersect the current level with this letter's match restriction
    -union the current level with the next lowest level
    -repeat for all lower levels

For > 0 removed characters:
    -subtract the number of removed characters from the match condition
       -intersect with the match condition?
    -union the current level with the lower level's rmv chars that's
      1 less than this level's numb rmved characters
    -repeat for all lower levels, then for lower rmved characters.

Note: This solution will take a ton of these iterations, and may not be
    the best solution. Maybe,you could do this all in 1 set and have a variable
    on each potential needle that tells how many removed characters there were
    and add up to total allowed removable characters each time.
    note: intersection and union work on dictionaries,
    make sure to stop early with skipped letters
